...
LOG = OUT / 'train_run.log'

with open(LOG, 'w') as logf:
    logf.write('Starting short training test\n')
    try:
    ...
    try:
        model = YOLO(model_name)
            ...
            try:
                ...
                sys.exit(3)
    logf.write('Beginning training (1 epoch)\n')
    start = time.time()
    try:
        # short training: 1 epoch, small imgsz to speed up
        model.train(data='data/urpc.yaml', epochs=1, imgsz=384, batch=8, device=0, project=str(OUT), name='short_train', exist_ok=True)
    except Exception as e:
        logf.write(f'Training failed: {e}\n')
        print(f'Training failed: {e}', file=sys.stderr)
        sys.exit(4)
    elapsed = time.time() - start
    logf.write(f'Training completed in {elapsed:.1f}s\n')
print('Training test completed; logs and outputs in', OUT)
sys.exit(0)
